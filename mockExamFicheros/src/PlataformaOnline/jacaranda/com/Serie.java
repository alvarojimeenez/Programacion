...
import PlataformaOnline.jacaranda.com.comparadores.ordenaPorNotaMedia;
...
public class Serie {
	...
	for (Temporada t : this.temporadas) {
		if (t.getNombreTemporada().equals(nombreTemporada)) {
			throw new SerieException("Ya existe ese nombre de temporada");
		}else {
	...
	public String listadoTemporadasPorNotaMedia() {
		Collections.sort(this.temporadas, new ordenaPorNotaMedia());
		Collections.reverse(this.temporadas);
	...
	public String ordenarSeriePorAgnio() {
		Collections.sort(this.temporadas);
		return this.temporadas.toString();
	}
	...
	/*
	CUESTIÓN: ¿Por qué crees que para los capítulos se ha optado por una estructura de tipo lista?
			Justifica tu respuesta en el código de la clase Serie.
	*/
	...
	/*
	  Respuesta: Porque no puede haber dos capítulos con el mismo nombre. 
	  Si hubiesemos usado un conjunto podríamos asignar mas de una vez el mismo nombre.
	*/
}
